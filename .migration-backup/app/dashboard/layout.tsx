...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (pathname === '/dashboard/login' || pathname === '/dashboard/callback') {
      setLoading(false);
      return;
    }
    if (!getToken()) {
    ...
    <div className="min-h-screen flex bg-black text-white">
      <aside className="w-60 border-r border-orange-900/30 bg-gradient-to-b from-black to-[#1a0800] p-4 flex flex-col">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm transition ${
                  active ? 'bg-orange-600/20 text-orange-300' : 'text-gray-300 hover:bg-white/5'
                }`}
              >
        ...
        <div className="mt-auto pt-4 border-t border-white/10">
          ...
          <button
            onClick={() => { clearToken(); router.replace('/dashboard/login'); }}
