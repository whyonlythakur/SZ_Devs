            ...
            {bots.map((b) => (
              <tr key={b.id} className="border-t border-white/5 hover:bg-white/5">
                ...
                <td className="p-3 text-right space-x-2">
                  {p.visibility && !frozen && (
                    <button title={b.is_visible ? 'Hide' : 'Show'} onClick={() => dashApi.toggleVisibility(b.id, !b.is_visible).then(load)} className="p-1.5 rounded hover:bg-white/10">
                  ...
                  {p.del && !frozen && (
                    <button title="Delete" onClick={() => { if (confirm(`Delete "${b.title}"?`)) dashApi.deleteBot(b.id).then(load); }} className="p-1.5 rounded hover:bg-red-500/20 text-red-400">
...
function BotEditor({ initial, isEdit, onClose, onSaved }: any) {
  ...
  const inputClass = "w-full bg-white/5 border border-white/10 rounded-md px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-600";
      ...
      <div className="bg-[#1a0800] border border-orange-900/40 rounded-xl max-w-3xl w-full max-h-[90vh] overflow-auto">
        ...
        <div className="p-5 space-y-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="File link (Mega/Drive)"><input value={form.filelink || ''} onChange={(e) => set('filelink', e.target.value)} className={inputClass} /></Field>
          </div>
          <div className="flex gap-6">
            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={!!form.is_visible} onChange={(e) => set('is_visible', e.target.checked)} /> Visible to public</label>
            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={!!form.featured} onChange={(e) => set('featured', e.target.checked)} /> Featured</label>
          </div>
