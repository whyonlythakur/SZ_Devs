            ...
            {staff.map((s) => {
              ...
              return (
                <tr key={s.discord_id} className="border-t border-white/5">
                  ...
                  <td className="p-3">
                    {s.is_frozen ? <span className="text-blue-300 text-xs flex items-center gap-1"><Snowflake className="h-3 w-3" /> Frozen</span> : <span className="text-green-400 text-xs">Active</span>}
                  </td>
                  <td className="p-3 text-right space-x-2">
                    ...
                    {showUnfreeze && (
                      <button onClick={() => act(() => dashApi.unfreeze(s.discord_id))} className="px-3 py-1 text-xs rounded bg-green-600 hover:bg-green-700 flex items-center gap-1 inline-flex"><Sun className="h-3 w-3" /> Unfreeze</button>
                    )}
                  </td>
                </tr>
              );
